...
def generate_iv_stratified_validation_list(predictions, helper_lipid_names, cell, name, save_path):
  ...
  # sns.histplot(predictions, x =f'{cell}_Prediction', bins = np.linspace(0,1, endpoint = True, num = N_bins+1))
  # plt.show()
...
##################### Run Predictions ###############################
  ...
  #     formulation_list = generate_iv_stratified_validation_list(prediction_list, helper_lipids, cell_type, model_name, save_path)
  #     formulation_list["Pred_Rank"] = formulation_list[f"{cell_type}_Prediction"].rank(ascending = False) #Add ranking
