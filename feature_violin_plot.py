...
save_path = 'Predictions/Yining_manuscript/'
wt_percent = False
if wt_percent == True:
...
##################### Run Predictions ###############################
  ...
  training_data = df.loc[:,df.columns.isin(['Formula label', 'Helper_lipid'] + input_param_names + ['RLU_DC24'])]
  formatted_training_data = training_data.dropna()
  formatted_training_data.reset_index(drop = True, inplace=True)
  training_data = formatted_training_data

  X = training_data.loc[:, training_data.columns.isin(input_param_names)]                         
  y = training_data['RLU_DC24'].to_numpy()
